...
while msg_count <= msg_total:
	topic = "mqtt/Android/pub/"+id_generator()[msg_count-1]+"/"
	payload = str(msg_count) + " Degree"
	msg_info = client.publish(topic, payload, qos=1, retain=False)
	time.sleep(2)
	if msg_info.is_published() == True:
		print("Topic 'mqtt/Android/pub/' all published done!")
	else:
		print("Topic 'Topic 'mqtt/Android/pub/' are not yet published.")
